...
public class RopeSegment : JumpThru {
    ...
    private static void PlayerUpdate(On.Celeste.Player.orig_Update orig, Player p) {
        ...
        if((p.StateMachine.State == 2 && p.DashDir.Y < 0) || p.CollideCheck<RopeSegment>(p.Position + new Vector2(0, -CORRECTION_BELOW))) {
        ...
        if(bridgeInside.Count > 0) {
            ...
            p.Speed.Y = 0;
        } else if(p.StateMachine.State == 0 && p.Speed.Y >= 0) {
